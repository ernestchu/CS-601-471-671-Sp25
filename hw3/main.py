...
def explore_mlp_activations(dev_d: Dict[str, List[Union[str, int]]],
    ...
    for act, actn in zip(activations, activation_names):
        train_config = EasyDict({
            'batch_size': batch_size,  # we use batching for training
            'lr': lr,
            'num_epochs': num_epochs,  # the total number of times all the training data is iterated over
            'hidden_dims': hidden_dims,  # the number of neurons in each hidden layer
            'save_path': f'model_hidden_{actn}.pth',  # path where to save the model
            'embeddings': embedding_type,
            'num_classes': num_classes,
            'activation': act,  # non-linear activation function
        })
    ...
    visualize_configs(all_emb_epoch_dev_accs, activation_names, "Accuracy", "./all_activations_acc.png")
    visualize_configs(all_emb_epoch_dev_losses, activation_names, "Loss", "./all_activations_loss.png")
...
def explore_mlp_learning_rates(dev_d: Dict[str, List[Union[str, int]]],
    ...
    for lr, lrn in zip(lrs, lrs_names):
        train_config = EasyDict({
            'batch_size': batch_size,  # we use batching for training
            'lr': lr,
            'num_epochs': num_epochs,  # the total number of times all the training data is iterated over
            'hidden_dims': hidden_dims,  # the number of neurons in each hidden layer
            'save_path': f'model_hidden_{lrn}.pth',  # path where to save the model
            'embeddings': embedding_type,
            'num_classes': num_classes,
            'activation': activation,  # non-linear activation function
        })
    ...
    visualize_configs(all_emb_epoch_dev_accs, lrs_names, "Accuracy", "./all_lrs_acc.png")
    visualize_configs(all_emb_epoch_dev_losses, lrs_names, "Loss", "./all_lrs_loss.png")
    # your code ends here
